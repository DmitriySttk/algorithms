...
class Graph < Hash
  def search(name)
    ...
    # The "searched" Hash is how you keep track of
    # which people you've searched before.
    # We use a hash because hash lookups are fast!
    searched = {}
...
# we begin the search from vertex "you" and pass
# the match criterion in the block, then we tap the search result
graph.search('you') { |person| person_is_seller?(person) }.tap do |person|
  puts "#{person} is a mango seller!" if person
end
